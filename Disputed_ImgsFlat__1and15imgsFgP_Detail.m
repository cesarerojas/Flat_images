function [] = Disputed_ImgsFlat__1and15imgsFgP_Detail(RESULTS_folder, numCaptDevice, numInitialImg, numRefImgs, width, height) 
%Function that analyses flat images. Option Detail
% -------------------------------------------------------------------------
...
% The Detail option means to go deeper in the search for information. 
% For each disputed flat image, each table assumes that it belongs to a different capture device.
%
% INPUTS:
...
% If numRefImgs == 15
%   The pixel icero and PRNU of the averaged of 15 reference images are read, R. (see 1)
%   T and H are made the same size.
%   The Mahalanobis Distance, DM, is obtained for each capture device. 
% else
%   Read the only reference images and get the icero and PRNU
%   T and H are made the same size.
%   The Mahalanobis Distance, DM, is obtained for each capture device. 
      ...
      for i = 3 : numberDisputesFiles
         ...
         if numRefImgs == 15
             ...
             while (k <= numCaptDevice)
                ...
                DisputedVectorSize(k,1) = size(T,1);
                if (k == 1)
         ...
         count = 1;
         change_IR = 1;
         while (count <= size(DM,2)) 
             C = zeros (size(DM,1),size(DM,2)*3);
             fil = 1;
             for column = 1 : size(DM,2)
                 for row = 1 : DisputedVectorSize(column,1)
                     if (DM(row,change_IR)<DM(row,column))
                         C(row,fil)=1;
                     else
                         if (DM(row,change_IR)>DM(row,column))
                             C(row,fil+1)=1;
             ...
             col = 1;
             for h = 1 : size(C,2)/3
                 total(h,column_counts) = sum(C(1:DisputedVectorSize(change_IR,1),col))+sum(C(1:DisputedVectorSize(change_IR,1),col+2));
                 total(h,column_counts+1) = sum(C(1:DisputedVectorSize(change_IR,1),col+1));
                 col = col +3;
             end
             column_counts = column_counts + 2;  
             count = count + 1;
             change_IR = change_IR + 1;
         end
         if numRefImgs == 15
             filename  = sprintf('%s.mat', [RESULTS_folder,'\DETAIL D',int2str(numInitialImg),',CD0',int2str(i-2),' - Average_15RefImg - DM 1 a 1']);
         else
             filename  = sprintf('%s.mat', [RESULTS_folder,'\DETAIL D',int2str(numInitialImg),',CD0',int2str(i-2),' - 1RefImg - DM 1 a 1']);
         end
         save (filename, 'total'); 
         column_counts = 1;
         clear total
         clear DisputedVectorSize
     end
